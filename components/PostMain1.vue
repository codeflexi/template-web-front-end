<template>
    <div :id="`PostMain-${post._id}`" class="flex border-b py-6">
        <div @click="isLoggedIn(post.user)" class="cursor-pointer">
            <img class="rounded-full max-h-[60px]" width="60" :src="post.user.image?post.user.image:''">
        </div>
...
onMounted(() => {
    ...
    //         console.log('Element is playing' + post.value._id);
    //         video.value.play()
    //     } else {
    //         console.log('Element is paused' + post.value._id);
    //         video.value.pause()
    //     }
...
const displayPost = (post) => {
    ...
    setTimeout(() => router.push(`/post/${post._id}`), 200)
}
</script>